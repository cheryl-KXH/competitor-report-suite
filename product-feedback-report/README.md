...
社媒不再设置独立统计按钮，也不再读取“大众点评 / 微博 / 小红书 / 抖音 / B站”分平台附件列。唯一原始数据入口是消费者反馈表当前行的“社媒CleanedRawData”单个 `.xlsx` 附件，由“报告生成”任务统一读取和统计。工作簿必须包含“大众点评 / 微博 / 小红书 / 抖音 / B站”；如有“大众点评-截图”则直接忽略。任务会拼入实习生在“大众点评”工作表做好的统计表，其余四张工作表按“情感识别”判定整行全部标签及用户为好评或差评。

每个清洗工作簿只允许包含当前行的一款新品。产品名按全半角、大小写、空格和常见分隔符规范化后精确匹配，不使用简称或包含匹配；缺少必要工作表/列、存在空产品名、包含其他新品或文件不包含当前产品时会在生成前停止。微博、小红书、抖音和 B站的好评/差评用户数按对应情感的数据行数计算且不去重；关键词评论数按各行归入对应情感的标签出现次数计算。大众点评沿用工作表内的用户数和标签评论数汇总。消费者反馈右侧“总计”为五个平台好评用户数与差评用户数之和，好评率为好评用户数除以总计；主要提及关键词合并五个平台标签，分别剔除“好喝，喜欢，推荐”和“难喝，不喜欢，不推荐”后取好评、差评 Top 3。
...
每次报告生成都会自动生成 `{品牌}-{抓取日期}-产品清单.xlsx` 样件并上传到当前任务文件夹，不回填 AI 表“产品清单”字段。需要人工调整时，从任务文件夹下载该样件，修改后再上传到 AI 表“产品清单”字段；报告任务检测到人工附件后优先用人工版统计，同时仍会在文件夹中刷新自动样件。日期列除正常日期外提供 `提前下架`、`外卖无售` 两个人工选项。关注新品不在原始外卖清单时，可人工新增一行并选择对应状态；报告开头会显示 `产品名：提前下架，暂无月销数据。` 或 `产品名：外卖无售，暂无月销数据。`。

报告生成期间分为 6 步：`1/6 正在读取品牌、新品及报告日期`、`2/6 正在读取上传的外卖及社媒数据`、按产品清单情况显示 `3/6 正在自动标注在售不满30天的产品上新日期` 或 `3/6 正在生成产品清单样件并读取人工配置的产品清单`、`4/6 正在进行外卖及社媒数据统计`、`5/6 正在生成跟踪反馈报告`、`6/6 正在上传至钉钉文档`。完成反馈按问题类型合并，例如 `报告已生成：京东外卖无数据；[新品A][新品B]社媒无数据`。只要某新品至少一个社媒平台有数据，就不提示该新品的单个平台缺失；缺失美团或饿了么有效数据会停止生成。
...
钉钉 HTTP 调用 `/generate-report` 时，外卖数表、社媒统计表以及最终 HTML/PDF 报告都在系统临时目录中生成。HTML 和 PDF 会一并上传至任务文件夹，其中“跟踪反馈报告”字段只回写 PDF 链接，HTML 仅在文件夹中留档；临时文件上传后自动清理，不会在本机 `outputs/` 留副本。下方命令行生成方式未传 `--output-dir` 时仍写入本机 `outputs/`。

PDF 由 Chrome/Chromium/Edge 的无头打印功能生成。服务环境需安装其中一种浏览器；如果无法自动发现，可在 `config/report_rules.json` 的 `pdf.browserExecutable` 中配置可执行文件路径，或设置 `CHROME_BIN`。

```bash
python -m scripts.delivery.prepare_product_menu --record-id local-test --input-dir "/path/to/样例文件夹" --report-date 2026-07-21
...
`产品清单及上新日期.xlsx` 只需要核对 `近30日上新日期`。自动标注会查询“最小抓取日期减 29 日”至报告日期（含首尾当天）的同品牌新品；未填写日期的产品按完整 30 天计算。
